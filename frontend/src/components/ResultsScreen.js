...
const DISCLAIMER = 'PathBack provides information and drafting help, not legal advice. For legal advice, consult a licensed attorney.';

const TIER_LABELS = {
  claude: 'PathBack Core',
  groq: 'PathBack Fast Lane',
  local: 'PathBack Backup Model',
};

const ResultsScreen = ({ results, category, isLoading, adminToken, onStartNew, onBack, onUpgrade }) => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'PathBack helped me fight back',
        text: 'I stood my ground with PathBack. When someone knocks you off course, PathBack helps you get back on your path.',
...
Truth · Safety · We Got Your Back
    ...
    a.href = url;
    a.download = 'pathback-response.txt';
    a.click();
  ...
  return (
    <div className="results-screen">
      ...
      <div className="results-container">
        {/* BACKUP-MODEL CAUTION (local lane) */}
        {results.notice && (
          <div style={{
        ...
        {isComplete && (
          <>
            ...
            {results.remaining_responses !== 'unlimited' && (
              <div className="remaining-count">
                {results.remaining_responses === 0 ? (
                  "You've used your 3 free responses for today."
                ) : (
                  `${results.remaining_responses} free responses remaining today`
                )}
              </div>
            )}
            {results.remaining_responses === 0 && onUpgrade && (
            ...
            <div className="results-footer">
              ...
              {results.served_by && (
                <div style={{
                  ...
                  Served by {TIER_LABELS[results.served_by] || results.served_by}
                  {results.downgraded ? ' (temporary fallback)' : ''}
                </div>
              )}
              {results.version && (
                <div style={{
