...
class GuardResponse:
    """Mimics requests.Response surface used by the suite, populated from SSE."""
...
def _parse_guard_sections(text):
    sections = {"wait": [], "leverage": "", "guard_steps": []}
    markers = [("===WAIT===", "wait"), ("===LEVERAGE===", "leverage"), ("===GUARD_STEPS===", "guard_steps")]
    found = []
    for marker, key in markers:
        idx = text.find(marker)
        if idx != -1:
            found.append((idx, idx + len(marker), key))
    found.sort()
    for i, (_, end, key) in enumerate(found):
        ...
        if key == "leverage":
            ...
            sections[key] = [line.strip() for line in block.split("\n") if line.strip()]
    return sections


def guard_post(payload, timeout=60):
    """POST to /api/guard, consume SSE stream, return a response-like object."""
    if ADMIN_TOKEN and isinstance(payload, dict) and "admin_token" not in payload:
        payload = {**payload, "admin_token": ADMIN_TOKEN}
    r = requests.post(f"{BASE}/api/guard", json=payload, stream=True, timeout=timeout)
    ...
    for raw in r.iter_lines(decode_unicode=True):
        ...
        etype = evt.get("type")
        if etype == "text":
            full_text += evt.get("chunk", "")
        elif etype == "done":
            final = evt
        elif etype == "error":
            return GuardResponse(500, {"error": evt.get("message", "stream error")}, full_text)

    sections = _parse_guard_sections(full_text)
    data = {
        **sections,
        "plan": final.get("plan"),
        "remaining_responses": final.get("remaining_responses"),
    }
    if final.get("citation_warning"):
        data["leverage"] = (data["leverage"] + f"\n\nNote: {final['citation_warning']}").strip()
    return GuardResponse(200, data, full_text)
...
def test_basic_employment():
    ...
    assert r.status_code == 200
    data = r.json()
    has_leverage = 'leverage' in data and data['leverage']
    has_steps = 'guard_steps' in data and data['guard_steps']
    keys = list(data.keys())
...
def test_basic_housing():
    ...
    assert r.status_code == 200
    data = r.json()
    ok = bool(data.get('leverage')) and bool(data.get('guard_steps'))
    return ok, f"State: Texas — got leverage: {bool(data.get('leverage'))}"
...
def test_all_categories():
    ...
    for cat in categories:
        ...
        if r.status_code != 200 or not r.json().get('leverage'):
            failed.append(cat)
        time.sleep(1)
    ok = len(failed) == 0
...
def test_missing_category_still_works():
    ...
    ok = r.status_code == 200 and bool(r.json().get('leverage'))
...
def test_response_not_hedging():
    ...
    assert r.status_code == 200
    data = r.json()
    leverage = (data.get('leverage') or '').lower()
